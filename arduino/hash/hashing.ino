...
void setup() {
  ...
  String InputMessage = "The quick brown fox" ;
  Serial.begin(9600);
  Serial.println();
  Serial.print("SHA256 hash program version 1.1 on Arduino Nano board") ;
  Serial.println();
  Serial.print("Now support UTF-8 with maximum printable characters up to 55 bytes");
  Serial.println();
  Serial.print("Written by p'SorSixty");
  Serial.println();
  Serial.print("5 January 2020");
  Serial.println();
  Serial.println();
...
// Function ProcessInputMessage
  unsigned int S0; // Sigma0(HashA)  
  unsigned int S1; // Sigma1(HashE)  
  unsigned int maj; // Majority (A,B,C)
  unsigned int ch; // Choose (E,F,G)
  unsigned int temp1, temp2;
  unsigned int w[64]; //Holds input message
  unsigned int s0[64]; //Numbers to generate w[i]
  unsigned int s1[64]; //Number to generate w[i]
  
// Initial hash value
  unsigned int a, h0=0x6A09E667;
  unsigned int b, h1=0xBB67AE85;
  unsigned int c, h2=0x3C6EF372;
  unsigned int d, h3=0xA54FF53A;
  unsigned int e, h4=0x510E527F;
  unsigned int f, h5=0x9B05688C;
  unsigned int g, h6=0x1F83D9AB;
  unsigned int h, h7=0x5BE0CD19;
...
// Initial array of round constants:
  ...
  Serial.print("NoOfWordsUsed: ");
  Serial.println(NoOfWordsUsed);
  ...
  Serial.print("ByteInWord2WriteBit1: ");
  Serial.println(ByteInWord2WriteBit1);
...
// Function GetS1 (s1[i])
  unsigned int s1Return;
  unsigned int Gets1 (unsigned int wi2Receive) {
  unsigned int debug = wi2Receive;
  Serial.print("Second binary: ");
  Serial.println((debug>>17));

  Serial.print("BITWISE AND BETWEEN ");
  Serial.print(debug);
  Serial.print(" AND ");
  Serial.println(0x0001ffff);
 // s1Return=((wi2Receive & 0x0001ffff)<<32-17)+(wi2Receive>>17)^((wi2Receive & 0x0007ffff)<<32-19)+(wi2Receive>>19)^(wi2Receive>>10);
  s1Return=((wi2Receive & 0x0001ffff)<<32-17)+(wi2Receive>>17)^((wi2Receive & 0x0007ffff)<<32-19)+(wi2Receive>>19)^(wi2Receive>>10);
  //Serial.println(sizeof(s1Return));

  return s1Return;}

// Function Gets0[i]
  unsigned int s0Return;
  unsigned int Gets0 (unsigned int wi15Receive) {
...
// Function GetChoose(E,F,G)
  unsigned int Choose ;
  unsigned int GetChoose (unsigned int HashE,unsigned int HashF,unsigned int HashG) {
  Choose=(HashE & HashF)^((~HashE)&HashG);  
  return Choose;}
    
// Function GetMajority
  unsigned int Majority; //Majority(A,B,C)
  unsigned int GetMajority (unsigned int HashA,unsigned int HashB,unsigned int HashC) {
...
// Function GetSigma0
  unsigned int TempSigma0 ;
  unsigned int GetSigma0 (unsigned int InputHash) {
...
// Function GetSigma1
  unsigned int TempSigma1 ;
  unsigned int GetSigma1 (unsigned int InputHash) {
...
// Function PrintWithLeadingZero
  unsigned int TempLz;
  void PrintWithLeadingZero (unsigned int ReceiveUnsignedLong) {
